...
\begin{abstract}
Regression adjustment is widely used to estimate average treatment effects in randomized trials. Simple regression (REG, ANCOVA) and interacted regression (IREG, ANCOVA II) have the same first-order variance under balanced assignment, so first-order theory cannot choose between them. In a fixed-dimensional superpopulation framework we derive a closed form for the difference in the $1/n^2$ terms of their mean-squared-error expansions under complete randomization. It depends on covariate shape, treatment-effect heterogeneity, residual heteroskedasticity and nonlinear misspecification, and can favor either estimator; the resulting asymptotic deficiency is a few dozen observations or fewer in our moderately skewed designs and is largest under extreme skewness, where the finite-sample gaps are large but the expansion no longer predicts their magnitude. As a practical companion, we show by simulation that without treatment-effect heterogeneity, interacted regression with HC1 standard errors can undercover substantially under skewed, heteroskedastic designs, in a pattern consistent with high within-arm leverage, while simple regression does not in our designs; HC3 largely corrects the variance scale but not the most extreme leverage configurations. We also note that the conventional robust variance of the centered interacted implementation omits a superpopulation variance component, and show that its plug-in correction is accurate when heterogeneity is large but miscalibrated when heterogeneity is small.
\end{abstract}
...
To see what may be at stake in practice, consider an example modeled after a typical tech industry setting. Say a company runs an A/B test on a sample of its user base and adjusts for $X_i$, each user's own engagement level from the prior period (e.g., number of clicks). Such engagement measures are right-skewed because a small number of ``super-users'' generate far more activity than typical users. Suppose also that these high-engagement users generate more volatile outcomes, so that outcome noise increases with $X_i$. Specifically, let the raw $X_i$ be marginally $\mathrm{LogNormal}(0,0.8^2)$ and let
\[
\sigma(X_i)=1+2\,X_i,
\qquad
Y_i(0)=0.9\,X_i+\sigma(X_i)\varepsilon_i,
\qquad
Y_i(1)=Y_i(0)+1+0.4\,X_i,
\]
with $\varepsilon_i\overset{\mathrm{iid}}{\sim}N(0,1)$. Thus, the treatment effect itself varies with prior engagement, while the variance of the outcome is substantially larger for the users in the right tail. Here the whitened covariate has skewness $3.69$ and kurtosis $34.4$, and the heterogeneity share $R^2_\tau$ defined in Section \ref{sec:asymptotics} is $0.0032$. Our second-order formula gives an asymptotic deficiency of $\Delta = 15.6$ for IREG relative to REG: asymptotically, using IREG costs about sixteen users' worth of precision. At $n=500$ this predicts that IREG's mean-squared error should exceed REG's by $3.1\%$, and simulation gives $2.3\%$ (Monte Carlo standard error $0.02\%$); at $n=2{,}000$ the prediction is $0.78\%$ and simulation gives $0.67\%$. The HC1 intervals of both estimators are close to nominal here at $n=500$: IREG+HC1 covers the population ATE in $94.5\%$ of replications and REG+HC1 in $95.0\%$.

Two features of this example recur throughout the paper. First, the effect is real but bounded: a deficiency of sixteen observations matters at $n=500$ and is negligible at $n=50{,}000$. Second, the size of the deficiency is governed by high-order moments of the covariate distribution, and those same moments govern how quickly the second-order approximation becomes accurate. If we instead take $X_i \sim \mathrm{LogNormal}(0,1.8^2)$ with $\sigma(X_i)=1+1.5X_i$ and a treatment-effect slope of $0.3$---a tail heavy enough that the whitened covariate has skewness $136$---then the formula returns $\Delta \approx 3.9\times10^4$ and predicts an MSE ratio of $78.5$ at $n=500$, whereas the simulated ratio is $1.68$. In that regime the expansion gets the ordering right and substantially overstates the numerical gap; Appendix \ref{sec:validity} quantifies where the crossover occurs in a closely related scalar family, with $\sigma(X)=1+X$ and slope $0.4$, which differs from both examples in its heteroskedasticity.

Motivated by this gap, we conduct a higher-order asymptotic analysis of both simple and interacted regression adjustments in RCTs to elucidate the relative merits of the two approaches when $\pi = 0.5$. We frame our findings in terms of asymptotic deficiency \citep{HodgesLehmann1970}. When two estimators have matching first-order behavior but one dominates the other on the second-order scale then, asymptotically, switching to the better one gives an accuracy boost that is equivalent to a fixed boost in sample size, which is referred to as the deficiency. Our results show that there is no universal second-order ordering: the comparison depends on covariate shape, treatment-effect heterogeneity, residual heteroskedasticity and nonlinear misspecification, and it can favor either estimator. The combination that favors simple regression in our examples is skewness interacting with heteroskedasticity or misspecification, as in the multiplicative error model commonly encountered in tech settings; simple regression can outperform interacted regression even when treatment-effect heterogeneity is absent. In the special case of Gaussian covariates with homoskedastic, well-specified errors the second-order coefficient instead favors IREG, and in the fixed-$p$ asymptotic framework it is linear in the number of covariates $p$. This qualitative pattern runs against the finite-sample parameter-count heuristic emphasized by \citet{zhao2023covariate}, though the magnitude involved is exactly $(p+1)R^2_\tau$ observations in that case, and fixed-$p$ asymptotics cannot speak to regimes in which the parameter count grows with $n$, which is the regime that motivates their concern.

As a practical companion to this theory, we study confidence intervals from standard software, \texttt{lm\_lin} for IREG and \texttt{lm\_robust} for REG from the R package \texttt{estimatr}. In simulations with $\gamma=0$, where treatment effects are constant and there is no heterogeneity to account for, IREG with HC1 standard errors can undercover substantially when a covariate is skewed and residual variance grows in its tail, while REG with HC1 stays close to nominal in the designs we examine. The pattern is consistent with a leverage mechanism, since the interacted specification fits each arm separately. HC3 substantially improves IREG's coverage in most of these designs, but it remains below nominal under very heavy-tailed covariates and is unreliable when a treatment arm contains an observation with leverage one. In the null-heterogeneity design with the most skewed covariate, HC1's failure is primarily an understatement of the variance scale, which HC3 largely removes, leaving smaller distortions in the tails. Separately, under superpopulation sampling the variance of centered IREG for the population average treatment effect contains a component that its conventional robust variance does not include; it matters when covariate-explained heterogeneity is large. In a diagnostic, its feasible plug-in estimate tracks the true component when heterogeneity is large but is noisy and miscalibrated when heterogeneity is small or absent, so coverage gains from it need not reflect the estimand-level component (Section \ref{sec:estimand} and Appendix \ref{sec:correction}).
...
Both regression estimators are smooth functions of sample moments, and for covariate distributions with atoms the relevant Gram matrices are singular with positive probability, so their exact second moments need not exist. We therefore work throughout with the second moment of the stochastic expansion rather than with the exact MSE, and we are explicit about the distinction.

\begin{defi}[Expansion MSE]
\label{def:msexp}
Let $V_i$ collect the sample-moment coordinates entering the coefficient map of an estimator, let $\mu = \EEE[V_i]$, $\bar V = n^{-1}\sum_i V_i$, and suppose $\htau = g(\bar V)$ for a function $g$ that is three times continuously differentiable in a neighborhood of $\mu$ with $g(\mu) = \tau$. Write $Z^{(n)} = \sqrt n(\bar V - \mu)$ and let
\begin{equation}
\label{eq:hall_expansion}
\tilde\tau_n
= \tau
+ \frac{1}{\sqrt n} a_i Z^{(n)}_i
+ \frac{1}{2n} H_{ij}Z^{(n)}_iZ^{(n)}_j
+ \frac{1}{6n^{3/2}}A_{3,ijk}Z^{(n)}_iZ^{(n)}_jZ^{(n)}_k
\end{equation}
be the third-order Taylor polynomial of $g$ at $\mu$, with $a$, $H$ and $A_3$ the first three derivative tensors of $g$ at $\mu$ and the Einstein summation convention in force. The expansion MSE is $\MSEexp(\htau) = \EEE[(\tilde\tau_n-\tau)^2]$.
\end{defi}

Definition \ref{def:msexp} presumes that the $V_i$ are IID, as they are under Bernoulli assignment. Under complete randomization the assignments are dependent; there the observations in the two arms form independent IID samples (Lemma \ref{lemm:two_sample} in Appendix \ref{sec:proofs}), each estimator is a smooth function of the two per-arm sample means, and $\MSEexp$ is the second moment of the third-order Taylor polynomial of that function at the per-arm population means. $\MSEexp$ is a finite sum of moments of sample means and is well defined whenever those moments exist; it is the object that the deficiency calculations of \citet{Rothenberg1984} and \citet{Hall1992} manipulate. It is not the exact MSE. Conditions under which formal expansions of this kind describe suitably trimmed exact moments are given by \citet{Sargan1974}; we do not verify them here, and our Monte Carlo checks use the untrimmed empirical MSE, reporting the frequency of degenerate draws (Appendix \ref{sec:numerical-verification}).
...
Assumption \ref{assu:moments} is a convenient sufficient condition rather than a sharp one: the coefficients of Theorem \ref{theo:deficiency} involve moments of $Z$ of order at most four and mixed moments such as $\EEE[Z\varepsilon^2]$, while $\MSEexp$ additionally requires the sixth moments of the moment vector $V_i$, whose entries are products of at most two coordinates of $(X_i, Y_i)$. Assumption \ref{assu:moments} implies $\EEE\|V_i\|^6<\infty$ by the Cauchy--Schwarz inequality.
...
\]
\[
E=\EEE[ZZ^\top\varepsilon],
\qquad
E_{W}=\EEE[ZZ^\top(W-\tfrac12)\varepsilon] = \tfrac14\big(\EEE[ZZ^\top\varepsilon(1)]-\EEE[ZZ^\top\varepsilon(0)]\big).
\]
Then, with $\MSEexp$ as in Definition \ref{def:msexp} and under the balanced CRT,
\[
\MSEexp(\hat\tau_{\mathrm{IREG}}) - \MSEexp(\hat\tau_{\mathrm{REG}}) = \frac{D_p}{n^2}+O(n^{-5/2}),
\]
where
\[
\begin{aligned}
...
-8(\gamma^\top S)\tr(E_W)
+8\gamma^\top E_WS
+8\tr(E^2)+8S^\top \EEE[Z\varepsilon^2].
\end{aligned}
\]
...
The proof is in Appendix \ref{sec:proofs}. Under complete randomization the two arms are independent IID samples (Lemma \ref{lemm:two_sample}), which yields an exact expansion of the conditional expansion MSE at any treatment fraction (Lemma \ref{lemm:conditional}); the proof then combines a reduction from the completely randomized design to Bernoulli assignment (Theorem \ref{theo:crt_reduction}) with the Bernoulli calculation (Theorem \ref{theo:bern_deficiency}), whose index algebra is certified symbolically by a script in the replication repository.

The expression for $D_p$ combines four blocks: a covariate shape block scaled by the heterogeneity direction $\gamma$; terms crossing $\gamma$ with residual asymmetry; residual heteroskedasticity; and nonlinear misspecification. None of these alone dictates the ordering. In particular, the deficiency need not favor REG even when there is no treatment-effect heterogeneity: when $\gamma=0$ all terms involving $\gamma$ vanish and
\[
D_p=8\tr(E^2)+8S^\top \EEE[Z\varepsilon^2].
\]
While $\tr(E^2) = \|E\|_F^2 \geq 0$, the term $S^\top \EEE[Z\varepsilon^2]$ can have either sign, so $\Delta$ can be positive or negative in this case.

\begin{rema}[Bias and variance components]
\label{rema:bias}
Two of the terms in $D_p$ are a squared-bias difference. Let $\tilde\tau_n$ be the Taylor polynomial of Definition \ref{def:msexp}, expanded under complete randomization in the per-arm sample means of Lemma \ref{lemm:two_sample}. Its linear term has mean zero; its quadratic term has mean $\frac12\tr(H\Sigma_n)$, where $H$ is the Hessian of the estimator's coefficient map and $\Sigma_n$ the covariance of the per-arm means, which is exactly proportional to $1/n$; and its cubic term has mean $O(n^{-2})$, because third central moments of sample means are third cumulants divided by squared sample sizes. The expansion bias is therefore $\EEE[\tilde\tau_n-\tau]=\frac12\tr(H\Sigma_n)+O(n^{-2})$, and evaluating the Hessians of the two estimators (Appendix \ref{sec:proofs}) gives
\[
\EEE[\tilde\tau_{n,\REG}-\tau] = -\frac{4\tr(E_W)+\gamma^\top S}{n}+O(n^{-2}),
\qquad
\EEE[\tilde\tau_{n,\IREG}-\tau] = -\frac{4\tr(E_W)}{n}+O(n^{-2}),
\]
so that $n^2\{\mathrm{Bias}^2_{\IREG}-\mathrm{Bias}^2_{\REG}\}=-(\gamma^\top S)^2-8(\gamma^\top S)\tr(E_W)$ to leading order, exactly the $-Q-8B_P$ part of $D_p$ in the notation of Appendix \ref{sec:proofs}; the remaining terms are variance contributions. These are statements about the expansion, not about the exact finite-sample bias; Appendix \ref{sec:numerical-verification} compares them with simulated biases.
\end{rema}

To interpret the scale of $\Delta$ we use the heterogeneity share
\[
R^2_\tau
=
...
\begin{coro}
\label{coro:homosk}
In addition to Assumption \ref{assu:moments}, suppose the potential outcomes satisfy a well-specified linear model with homoskedastic errors, $\EE{\varepsilon_i(w)\cond X_i}=0$ and $\EE{\varepsilon_i^2(w)\cond X_i}=\sigma_w^2$ for $w\in\{0,1\}$. Then $E = E_W = 0$ and $\EEE[Z\varepsilon^2]=0$, so
\begin{equation}
...
\[
Y_i(w) = \alpha^*_{(w)} + X_i^\top\beta^*_{(w)} + \eta_i\|Z_i\|_2,
\]
...
It is useful to consider how the terms in \eqref{eq:mult} behave. The treatment-effect signal relative to residual variance is small in many applications, so terms multiplied by $R_\tau^2$ rarely dominate unless the moment factors are large. The bracketed component is negative and of order $p$ in the leading cases we have examined---it equals $-(p+1)$ under Gaussian covariates and $\kappa_j-p-4$ in the setting of Remark \ref{rema:kurtosis}---but its numerator grows with the higher moments of the covariate distribution, so it is sensitive to departures from Gaussianity in either direction.

The remaining term is nonnegative, since $8\nu^2\|S\|^2/(\|\gamma\|^2+4\nu^2p)\geq0$, and it vanishes only when the whitened covariates have no third-moment skewness in the direction $S=\EEE[Z\|Z\|^2]$. Covariates that are bounded below before centering, such as measures of user activity, engagement or spending, typically have large $\|S\|$, and the term can then dominate the covariate-shape block. The scalar case makes this transparent: with $p=1$, Corollary \ref{coro:mult_normproportional} gives
\[
\Delta
=
...
\begin{rema}[Nearly balanced designs]
\label{rema:local}
Exact balance is not needed for Theorem \ref{theo:deficiency} to be informative. Under Assumption \ref{assu:moments} (with Lemma \ref{lemm:conditional}), if $n_1/n = 1/2 + h/\sqrt n$ for fixed $h$, then by Proposition \ref{prop:first_order} the first-order variance gap between the two estimators is $-16h^2\|\gamma\|^2/n + O(n^{-3/2})$, so that
\[
\MSEexp(\htau_\IREG)-\MSEexp(\htau_\REG) = \frac{D_p - 16h^2\|\gamma\|^2}{n^2}+O(n^{-5/2}).
\]
Imbalance of order $\sqrt n$ units therefore shifts the comparison toward IREG by a known amount; the $4\|\gamma\|^2$ term relating the Bernoulli and CRT designs in Theorem \ref{theo:crt_reduction} is the special case in which $h^2$ is replaced by its expectation $1/4$ under Bernoulli randomization.
\end{rema}

\section{Feasible Inference}
\label{sec:feasible}

The second-order results concern point estimation. In practice both estimators are reported with confidence intervals, and the behavior of those intervals depends on the variance estimator as much as on the estimator. This section studies the standard implementation. Its findings are simulation evidence, subordinate to the theory of Section \ref{sec:asymptotics}.

\subsection{Standard implementation}
\label{sec:standard}

We use the R package \texttt{estimatr} \citep{estimatr}. REG is \texttt{lm\_robust(y \textasciitilde{} w + x)}. IREG is \texttt{lm\_lin(y \textasciitilde{} w, covariates = \textasciitilde{} x)}, which centres the covariates at their sample means, interacts them with treatment, and reports $\htau_\IREG$ as its coefficient on $w$. For a regression with $k$ columns, residuals $\hat e_i$ and leverages $H_{ii}$, HC1 scales the squared residuals in the sandwich by $n/(n-k)$, HC2 by $1/(1-H_{ii})$ and HC3 by $1/(1-H_{ii})^2$; intervals use $t$ critical values with $n-k$ degrees of freedom. HC1 is Stata's default, and \citet{Kranz2024} finds that $98.1\%$ of 40,571 heteroskedasticity-robust Stata regressions in reproduction packages of leading economics journals use it; HC2 is the default of \texttt{lm\_robust} and \texttt{lm\_lin} without clustering, motivated by its equivalence with Neyman's variance estimator \citep{samii2012equivalencies}, so our HC1 results correspond to requesting \texttt{se\_type = "HC1"} explicitly; HC3 approximates the jackknife \citep{MacKinnonWhite1985} and is recommended for small and moderate samples by \citet{LongErvin2000}. The downward bias of the unadjusted sandwich is governed by the leverages \citep{ChesherJewitt1987}. Leverage matters more for IREG than for REG in the same data, because the interacted specification amounts to separate least-squares fits in the two arms, each on about $n/2$ units.

\subsection{A superpopulation caveat}
\label{sec:estimand}

For the centered IREG implementation considered here, the conventional robust covariance estimator treats $\bar X$ as fixed. Under the superpopulation sampling of Section \ref{sec:asymptotics}, $\htau_\IREG = \htau + \bar X^\top\hdelta$ with $\bar X$ random, and the first-order sampling variance of $\htau_\IREG$ around the population ATE contains an additional component $G/n$ that this estimator does not include, where
\[
G = \p{\beta^*_{(1)}-\beta^*_{(0)}}^\top\Sigma_X\p{\beta^*_{(1)}-\beta^*_{(0)}} = \|\gamma\|^2
\]
in the notation of \eqref{eq:whitening}, with $\Sigma_X=\Sigma=\Var{X_i}$; equivalently $G = R^2_\tau V^*$. The need for such a component is discussed by \citet{imbens2009recent}, \citet{negi2021revisiting}, \citet{zhao2021covariate} and \citet{ye2023toward}. The omission is specific to this implementation. To first order both estimators satisfy $\htau-\tau=\gamma^\top\bar Z+\bar\varepsilon_1-\bar\varepsilon_0+o_P(n^{-1/2})$, with $\bar Z$ the full-sample mean of $Z_i$ and $\bar\varepsilon_w$ the arm-$w$ mean of $\varepsilon_i$, so both have first-order variance $V^*/n$. The residuals of the interacted fit are approximately $\varepsilon_i$, so its robust variance estimates $(V^*-G)/n$. REG omits the interactions, its residuals are approximately $\varepsilon_i+(W_i-\frac12)\gamma^\top Z_i$, and at $\pi=\frac12$ its robust variance of the treatment coefficient is approximately $4n^{-2}\sum_i\hat e_i^2$, which estimates $(4\EE{\varepsilon_i^2}+G)/n=V^*/n$; in the large-heterogeneity design of Appendix \ref{sec:correction}, REG+HC1 covers the population ATE in \gz{corr:large_hte_n500_p5:REG_HC1:0.95}\% of replications. Omitting $G/n$ gives a nominal $95\%$ IREG interval asymptotic coverage $2\Phi(1.96\sqrt{1-R^2_\tau})-1$, which is \gz{analytic:cov:0.01}\% at $R^2_\tau=0.01$ and \gz{analytic:cov:0.1}\% at $R^2_\tau=0.1$: the component matters when covariate-explained heterogeneity is large relative to outcome noise and is negligible when it is small. A feasible first-order correction replaces $G$ by its plug-in estimate,
\begin{equation}
\label{eq:vsp}
\hat V_{\mathrm{SP}} = \hat V_\IREG + \frac{\hdelta^\top\hat\Sigma_X\hdelta}{n},
\end{equation}
with $\hdelta$ the fitted interaction coefficients and $\hat\Sigma_X$ the sample covariance of $X$.

The plug-in must be distinguished from the quantity it estimates, because it is biased upward in finite samples. To leading order $\EE{\hdelta^\top\hat\Sigma_X\hdelta} \approx G + \operatorname{tr}\{\Sigma_X\Var{\hdelta}\}$. With homoskedastic, well-specified errors of variance $\sigma^2$ in both arms, each arm's slope is estimated from about $n/2$ units, so $\Var{\hdelta}\approx 4\sigma^2\Sigma_X^{-1}/n$ and
\[
\EE{\hdelta^\top\hat\Sigma_X\hdelta/n} \approx \frac{G}{n} + \frac{4p\sigma^2}{n^2}.
\]
In the five Gaussian homoskedastic designs of the correction study (Appendix \ref{sec:correction}) this approximation matches the simulated mean of the plug-in to within \gz{bias:gauss:maxrelerr}\%. The second term does not depend on $G$: when $\gamma=0$ the plug-in consists of it entirely, and when $G$ is small it can exceed $G/n$. Under heteroskedasticity and heavy-tailed covariates $\Var{\hdelta}$ is larger, its leading form involving $\EE{\varepsilon_i^2(X_i-\mu)(X_i-\mu)^\top}$, and the leading-order approximation itself becomes less accurate; we use it as an explanation of the mechanism, not as an exact formula. A coverage change produced by \eqref{eq:vsp} therefore need not reflect the estimand-level component.

Appendix \ref{sec:correction} separates the two by also adding the true $G/n$ (an oracle correction, not a feasible procedure) on the same simulated datasets, at nominal levels from $80\%$ to $99\%$. With large heterogeneity ($R^2_\tau=\gz{corr:large_hte_n500_p5:R2}$) the plug-in averages \gz{corr:large_hte_n500_p5:ratio} times $G/n$, and the feasible and oracle intervals are both close to nominal at every level examined: at $95\%$, HC1 covers \gz{corr:large_hte_n500_p5:IREG_HC1:0.95}\%, HC1 plus $G/n$ \gz{corr:large_hte_n500_p5:IREG_HC1o:0.95}\% and HC1 plus the plug-in \gz{corr:large_hte_n500_p5:IREG_HC1c:0.95}\%, with a median interval \gz{corr:large_hte_n500_p5:wr} times as wide as HC1's. With $\gamma=0$ and a heavily skewed covariate, adding $G/n=0$ changes nothing, while the plug-in, which widens the median interval by a factor of only \gz{corr:lognormal_s2.0_prop:wr}, moves HC1 coverage at $95\%$ from \gz{corr:lognormal_s2.0_prop:IREG_HC1:0.95}\% to \gz{corr:lognormal_s2.0_prop:IREG_HC1c:0.95}\%. That agreement with nominal is a crossing point, not calibration: across nominal levels the plug-in interval is miscalibrated, covering \gz{corr:lognormal_s2.0_prop:IREG_HC1c:0.80}\% at $80\%$ and \gz{corr:lognormal_s2.0_prop:IREG_HC1c:0.99}\% at $99\%$. Since $G=0$ in this design, the plug-in's effect is finite-sample compensation for the understated HC1 variance, not recovery of a missing variance component. We therefore use the standard implementation in the rest of this section and do not propose \eqref{eq:vsp} as a general remedy.

\subsection{Coverage without treatment-effect heterogeneity}
\label{sec:gamma0}

The finite-sample phenomenon we study is distinct from the caveat above. Every design in this section has $\gamma=0$: $\beta^*_{(1)}=\beta^*_{(0)}$ and the two potential outcomes share their residual, so every unit has the same treatment effect $\tau=1$. Then $R^2_\tau = 0$ and $G = 0$ exactly, the sample and population average treatment effects coincide, and the component of Section \ref{sec:estimand} is identically zero. Undercoverage in these designs is therefore a finite-sample failure of the variance estimator, not an omitted estimand-level variance.

\paragraph{Designs.} Treatment is assigned by a balanced CRT with $n=500$ unless stated, and
\[
Y_i(0) = 0.9\textstyle\sum_{j} X_{ij} + g(X_i) + \sigma(X_i)\varepsilon_i,
\qquad Y_i(1) = Y_i(0) + 1,
\qquad \varepsilon_i\sim N(0,1).
\]
The base design has $p=1$, a raw right-skewed covariate $X_i\sim\mathrm{LogNormal}(0,s^2)$ such as prior engagement, $\sigma(X)=1+X$ and $g = 0$. We vary $s$, add misspecification $g$ with homoskedastic or heteroskedastic noise, vary $n$, and use $p=5$ with $X_{i1}$ lognormal, $X_{i2},\ldots,X_{i5}\sim N(0,1)$ and $\sigma = 1+X_{i1}$. Each design uses $20{,}000$ replications ($10{,}000$ at $n=2{,}000$ and $4{,}000$ at $n=8{,}000$); with $20{,}000$ replications the Monte Carlo standard error of every reported coverage is at most \gz{mcse:max20k} percentage points. The simulations call \texttt{estimatr}'s own fitting routine with design matrices built exactly as \texttt{lm\_robust} and \texttt{lm\_lin} build them; Appendix \ref{sec:r-validation} describes how this was validated.

\paragraph{Results.} Table \ref{tab:gamma0-skew} shows that IREG+HC1 coverage deteriorates steadily as the covariate becomes more skewed: from \gz{lognormal_s0.6_prop:IREG_HC1:all}\% at $s=0.6$ to \gz{lognormal_s1.5_prop:IREG_HC1:all}\% at $s=1.5$ and \gz{lognormal_s2.0_prop:IREG_HC1:all}\% at $s=2.0$. REG+HC1 stays between \gz{all:REG_HC1:min}\% and \gz{all:REG_HC1:max}\% in every design of this section. HC2 recovers part of the gap for IREG and HC3 most of it. Table \ref{tab:gamma0-other} extends the picture. With Gaussian covariates, quadratic misspecification is harmless; with a skewed covariate, a misspecification that grows in the tail produces undercoverage even with homoskedastic noise (\gz{lognormal_s1.0_quadstd:IREG_HC1:all}\% for IREG+HC1). In the base design with $s=1.2$, the undercoverage shrinks as $n$ grows, from \gz{lognormal_s1.2_prop_n200:IREG_HC1:all}\% at $n=200$ to \gz{lognormal_s1.2_prop_n2000:IREG_HC1:all}\% at $n=2{,}000$ and \gz{lognormal_s1.2_prop_n8000:IREG_HC1:all}\% at $n=8{,}000$, and it persists with $p=5$.

The pattern is consistent with the leverage mechanism of \citet{ChesherJewitt1987}. IREG is equivalent to separate least-squares fits in the two arms, each on $n/2$ units, so a rare extreme covariate value has roughly twice the leverage it would have in REG's pooled fit---the median of the largest IREG leverage rises from \gz{lognormal_s0.6_prop:medlev} at $s=0.6$ to \gz{lognormal_s2.0_prop:medlev} at $s=2.0$---and its arm's slope is pulled towards it, shrinking its residual exactly where $\sigma(X)$ is largest. HC1 does not undo this shrinkage; the factor $1/(1-H_{ii})^2$ in HC3 largely does.

\paragraph{Scale or shape.} Table \ref{tab:scale} examines the $s=2$ design more closely. If the studentized statistic $T=(\htau-\tau)/\widehat{\mathrm{SE}}$ behaved like a scaled $t$ variable, $T\approx c\,t_{n-k}$, a single scale $c$ read off the $95\%$ coverage would predict coverage at every level. For IREG+HC1, $c=\gz{diag:scale:IREG_HC1}$, and the prediction is within \gz{diag:scale:IREG_HC1:maxdev} points of the observed coverage at every level from $80\%$ to $99\%$: HC1's failure in this design is primarily an understatement of the variance scale. The ratio of the empirical $|T|$ quantile to the $t$ quantile nevertheless rises from \gz{diag:scale:IREG_HC1:0.80:qratio} at $80\%$ to \gz{diag:scale:IREG_HC1:0.99:qratio} at $99\%$, so the tails of $T$ are also heavier than those of a scaled $t$.
...
\begin{tabular}{lrr|ccc|cc}
\toprule
& & & \multicolumn{3}{c|}{\textbf{IREG}} & \multicolumn{2}{c}{\textbf{REG}} \\
Design & $n$ & Reps & HC1 & HC2 & HC3 & HC1 & HC3 \\
\midrule
...
\begin{table}[htbp]
\centering
\caption{Scale versus shape of the studentized statistic in the $\gamma=0$, $s=2$ design of Table \ref{tab:gamma0-skew}. The implied scale $c$ solves $2F_{t}(q_{0.975}/c)-1=$ observed $95\%$ coverage; ``predicted'' is $2F_t(q/c)-1$ at each level; the last row of each block divides the empirical quantile of $|T|$ by the corresponding $t$ quantile.}
\label{tab:scale}
\small
...
\begin{table}[htbp]
\centering
\caption{Monte Carlo standard deviation and MSE of $\htau_\IREG$ and $\htau_\REG$ against their median HC1 and HC3 standard errors, in the designs of Table \ref{tab:gamma0-skew}; $1+\Delta/n$ is the second-order prediction of Theorem \ref{theo:deficiency} for the same designs.}
\label{tab:sampling}
\small
\begin{tabular}{r|cc|cc|ccc}
\toprule
& \multicolumn{2}{c|}{Monte Carlo SD} & \multicolumn{2}{c|}{MSE ratio IREG/REG} & \multicolumn{3}{c}{Median standard error} \\
$s$ & IREG & REG & simulated & $1+\Delta/n$ & IREG HC1 & REG HC1 & IREG HC3 \\
\midrule
...
\begin{table}[htbp]
\centering
\caption{Coverage (\%) in the rare-value design ($\gamma=0$, $p=1$, $n=500$, $\rho=0.02$, $20{,}000$ replications) by replication type: all replications as the software reports them, replications with a full-rank interacted design, and the subset of full-rank replications in which an arm contains a leverage-one unit (\gz{rare_q0.02_prop:lev1}\% and \gz{rare_q0.02_homo:lev1}\% of all replications). IREG is shown under \texttt{estimatr} \gz{estimatr:v1} and the GitHub development version \gz{estimatr:v2}, which evaluate the $0/0$ HC2/HC3 contribution differently and, in the \gz{rare_q0.02_prop:rankdef}\% and \gz{rare_q0.02_homo:rankdef}\% of rank-deficient replications, drop different collinear columns; REG does not depend on the version. A replication without a finite interval counts as a non-cover.}
...
\subsection{Practical synthesis}
\label{sec:practical}

Across the designs considered, the point-estimation and inference results suggest two practically distinct regimes. When treatment-effect heterogeneity is small, the second-order comparison is driven by covariate shape, heteroskedasticity and misspecification, and in several of our skewed, heteroskedastic designs it favors REG; IREG can meanwhile encounter finite-sample leverage problems that make robust inference difficult, although HC3 substantially improves coverage in most of these designs. In this regime $G/n$ is small or zero, and the plug-in's bias term can be large relative to it. When heterogeneity is large, the second-order comparison moves in IREG's favor, and $G/n$ becomes an important part of the uncertainty about the population ATE for the centered IREG implementation. In our large-heterogeneity design, which is Gaussian, homoskedastic and well specified, Corollary \ref{coro:homosk} gives a deficiency of \gz{corr:large_hte_n500_p5:Delta} observations in IREG's favor, a real but modest second-order gain; the feasible correction closely tracks the oracle and gives near-nominal coverage at every nominal level examined, with intervals a median \gz{corr:large_hte_n500_p5:wr} times as wide as HC1's; and REG+HC1 covers the population ATE in \gz{corr:large_hte_n500_p5:REG_HC1:0.95}\% of replications without any correction. These patterns are a synthesis of the designs studied rather than a decision rule: the second-order comparison also depends on covariate moments, heteroskedasticity and misspecification, and $R^2_\tau$ is not directly observed.

For practice, analysts using interacted regression can inspect the largest within-arm leverage: when it is near one, IREG robust standard errors should be treated with caution, and REG or a coarser specification of the interaction is a reasonable alternative. These are statements about the designs examined here, not a general ranking of the two estimators.
...
\emph{Point-estimator efficiency.} REG and IREG have the same first-order variance when $\pi=1/2$, and Theorem \ref{theo:deficiency} gives the next term of the comparison. It depends on treatment-effect heterogeneity, covariate shape, residual heteroskedasticity and nonlinear misspecification, and it can favor either estimator. Under Gaussian covariates with homoskedastic, well-specified errors the second-order coefficient favors IREG and is linear in $p$ (for fixed $p$); with independent symmetric coordinates the ordering is decided by whether the kurtosis of the heterogeneity-driving covariate exceeds $p+4$; heteroskedasticity or misspecification add terms that can favor REG even without heterogeneity. In our moderately skewed designs the deficiency amounts to at most a few dozen observations; its largest values occur in the heavy-tailed regimes where Appendix \ref{sec:validity} indicates that the second-order approximation itself becomes unreliable. There the finite-sample MSE gaps are far from modest (the MSE ratio is $1.68$ in the heavy-tailed example of Section \ref{sec:intro} and \gz{samp:s2.0:mseratio} in the $\gamma=0$, $s=2$ design of Section \ref{sec:gamma0}), the expansion does not predict their magnitude, and they are hard to estimate precisely by simulation because the relevant moments are dominated by rare covariate values (Appendix \ref{sec:validity}); in every design of Table \ref{tab:validity} and in the heavy-tailed example the simulated comparison has the sign the second-order analysis predicts. Second-order efficiency alone therefore rarely settles the choice between the two estimators.
...
Throughout this appendix, Latin indices $i,j,k,l,a,b,c,d$ attached to $Z$, $T_3$, $T_4$, $E$ and $P$ refer to \emph{coordinates} of the whitened covariate vector and are summed over in the Einstein convention; the unit index is suppressed. We write $G = \|\gamma\|^2$.
...
\end{align}
write $S=q=\EEE[Z\|Z\|^2]$, and set $E=\EEE[ZZ^\top\varepsilon]$ and $P=\EEE[ZZ^\top(W-\tfrac12)\varepsilon]$. For the scalar contractions
\begin{align}
G&=\|\gamma\|^2,
&
...
Let $V_i$ denote the vector of raw-moment coordinates needed to evaluate the REG or IREG coefficient map, and adopt the notation of Definition \ref{def:msexp}, so that $\htau = g(\bar V)$ and $\tilde\tau_n$ is given by \eqref{eq:hall_expansion}. Let
\[
\Sigma^V_{ij}=\Cov(V_i,V_j),
\qquad
\mu^V_{3,ijk}=\EEE[(V_i-\mu_i)(V_j-\mu_j)(V_k-\mu_k)]
\]
be the covariance and third central-moment tensors of the moment vector. Expanding the square of \eqref{eq:hall_expansion} and using the second- and third-order moments of sample means gives the order-$n^{-1}$ correction to $n\MSEexp$ as $T_1+T_2+T_3$ with
\begin{equation}
\label{eq:hall_T1}
T_1=a_iH_{jk}\mu^V_{3,ijk},
\end{equation}
\begin{equation}
\label{eq:hall_T2_bern}
T_2 = \frac14 H_{ij}H_{kl}\p{\Sigma^V_{ik}\Sigma^V_{jl}+\Sigma^V_{il}\Sigma^V_{jk}+\Sigma^V_{ij}\Sigma^V_{kl}},
\end{equation}
\begin{equation}
\label{eq:hall_T3_bern}
T_3 = \frac13 a_iA_{3,jkl}\p{\Sigma^V_{ij}\Sigma^V_{kl}+\Sigma^V_{ik}\Sigma^V_{jl}+\Sigma^V_{il}\Sigma^V_{jk}},
\end{equation}
...
\theta_I=(0,0,0,\gamma).
\]
Writing $R=A^{-1}$ and $\theta=Rb$, differentiation of $A\theta=b$ gives the exact derivative identities
\[
D\theta[h] = R(db-dA\,\theta),
\qquad
D^2\theta[h_1,h_2] = -RdA_1R(db_2-dA_2\theta)-RdA_2R(db_1-dA_1\theta),
\]
\[
D^3\theta[h_1,h_2,h_3] = \sum_{\pi\in S_3} RdA_{\pi_1}RdA_{\pi_2}R(db_{\pi_3}-dA_{\pi_3}\theta).
\]
For IREG the additional sample-mean centering functional is differentiated together with the coefficient map, producing the corresponding product-rule terms.

Substituting these derivative tensors and the covariance and third-moment blocks of $V_i$ into \eqref{eq:hall_T1}--\eqref{eq:hall_T3_bern}, and reducing the resulting indexed sums using $\EEE[\omega_i]=0$, $\omega_i^2=1/4$, the four population normal equations for $\varepsilon_i$ and the symmetries of $T_3$ and $T_4$, gives
\begin{align}
\Delta T_1 &= 2G+2F+8Z_\gamma, \label{eq:dT1}\\
\Delta T_2 &= -3G-F-Q-R-8Z_\gamma-8A_P-8B_P, \label{eq:dT2}\\
\Delta T_3 &= -(2p+6)G-2R-8A_P+8C_P+8\tr(E^2)+8S^\top \EEE[Z\varepsilon^2], \label{eq:dT3}
\end{align}
where $Z_\gamma = \EEE[(\gamma^\top Z)\|Z\|^2\omega\varepsilon]$ with $\omega=W-\frac12$. The algebra in \eqref{eq:dT1}--\eqref{eq:dT3} is carried out symbolically: the reference implementation enumerates the full Cartesian products of the expansion directions, applies the derivative tensors above, and reduces the resulting indexed expressions using only tensor symmetries, dummy-index relabeling and the identities implied by $\EEE[ZZ^\top]=I_p$ and $\omega_i^2=1/4$, with the dimension $p$ kept symbolic. The symbolic remainder for each of the three rows is identically zero; the certificate is \texttt{verification/general\_p\_symbolic\_certificate.py} in the replication repository and runs in under a minute.

Summing the three rows, the $Z_\gamma$ terms cancel and \eqref{eq:Dbern} follows.
\end{proof}

Theorems \ref{theo:crt_reduction} and \ref{theo:bern_deficiency} give
\begin{equation}
\label{eq:unsimp}
D_p = -(2p+3)G+F-Q-3R-16A_P-8B_P+8C_P+8\tr(E^2)+8S^\top \EEE[Z\varepsilon^2],
\end{equation}
which Lemma \ref{lemm:matrix} rewrites in the matrix form of Theorem \ref{theo:deficiency}.

\begin{lemm}[Equivalent matrix form]
\label{lemm:matrix}
With $K$, $S$, $S_\gamma$, $E$ and $E_W$ as in Theorem \ref{theo:deficiency},
\[
G=\|\gamma\|^2,
\quad
F=\gamma^\top K\gamma,
...
\[
\|S_\gamma\|_F^2=\sum_{a,b}(S_\gamma)_{ab}^2=\gamma_k\gamma_\ell T_{3,abk}T_{3,ab\ell}=\gamma_k\gamma_\ell L_{k\ell}=R.
\]
Since $S_\gamma$ and $E_W$ are symmetric, $\tr(S_\gamma E_W)=(S_\gamma)_{jk}(E_W)_{jk}=\gamma_iT_{3,ijk}P_{jk}=A_P$; finally $B_P=(\gamma^\top q)\tr(P)=(\gamma^\top S)\tr(E_W)$ and $C_P=\gamma^\top Pq=\gamma^\top E_WS$.
\end{proof}

We close with an exact identity that is used by the variance-reduced verification of Appendix \ref{sec:numerical-verification} and that underlies Remark \ref{rema:bias}.

\begin{lemm}[Exact difference identity]
\label{lemm:exact_diff}
Let $\bar X_w$ and $S_w=\sum_{i:W_i=w}(X_i-\bar X_w)(X_i-\bar X_w)^\top$ denote the within-arm means and scatter matrices, let $\hbeta_w=S_w^{-1}\sum_{i:W_i=w}(X_i-\bar X_w)(Y_i-\bar Y_w)$, and write $\Delta_X=\bar X_1-\bar X_0$. Then, whenever $S_0$, $S_1$ and $S_0+S_1$ are invertible and $n_1=n_0$,
\[
\htau_\REG = \bar Y_1-\bar Y_0-\Delta_X^\top (S_1+S_0)^{-1}(S_1\hbeta_1+S_0\hbeta_0),
\qquad
\htau_\IREG = \bar Y_1-\bar Y_0-\Delta_X^\top \frac{\hbeta_1+\hbeta_0}{2},
\]
and consequently
\[
\htau_\IREG-\htau_\REG = \tfrac12\,\Delta_X^\top (S_1+S_0)^{-1}(S_1-S_0)(\hbeta_1-\hbeta_0) = O_P(n^{-1}).
\]
\end{lemm}

\begin{proof}
Both expressions for the estimators are the Frisch--Waugh--Lovell representations of \eqref{eq:estimators}: the coefficient on $X$ in the REG regression is the pooled within-arm slope $(S_1+S_0)^{-1}(S_1\hbeta_1+S_0\hbeta_0)$, and IREG fits the two arms separately, so with $n_1 = n_0$ the overall covariate mean satisfies $\bar X_1 - \bar X = \Delta_X/2 = -(\bar X_0 - \bar X)$. Subtracting,
\[
\htau_\IREG-\htau_\REG = \Delta_X^\top\Big[(S_1+S_0)^{-1}(S_1\hbeta_1+S_0\hbeta_0) - \tfrac12(\hbeta_1+\hbeta_0)\Big]
= \tfrac12\Delta_X^\top(S_1+S_0)^{-1}(S_1-S_0)(\hbeta_1-\hbeta_0).
\]
The stochastic order follows from $\Delta_X = O_P(n^{-1/2})$, $(S_1+S_0)^{-1}(S_1-S_0) = O_P(n^{-1/2})$ and $\hbeta_1-\hbeta_0 = O_P(1)$.
\end{proof}

In whitened coordinates the leading term of this difference is $\tfrac14\Delta_Z^\top(A_1-A_0)\gamma$, with $A_w$ the arm-$w$ mean of $ZZ^\top-I_p$; this is the control variate used in \eqref{eq:cv_identity} below, and taking expectations in Lemma \ref{lemm:exact_diff} and in the corresponding expansion of $\htau_\REG-\tau$ gives the bias expressions of Remark \ref{rema:bias}, which we verify numerically rather than prove.
...
\toprule
DGP & $D_p$ & $D_p-4G$ & $-(p+1)G$ & drop $-3R$ & drop $-Q$ & drop resid.\ & drop $E_W$ \\
\midrule
...
\paragraph{Tail sensitivity.} In the heaviest designs the deficiency is dominated by covariate values far in the tail. Table \ref{tab:tail} recomputes $1+\Delta/n$ at $n=500$ after truncating the lognormal law at a high quantile, for which the moments entering $\Delta$ have closed forms. Truncation at the $1-10^{-4}$ quantile already reduces $\Delta$ by one to three orders of magnitude and brings $1+\Delta/n$ to the same order of magnitude as the simulated ratio. The simulations draw between $10^7$ and $5\times10^7$ covariate values, so values beyond the $1-10^{-8}$ quantile rarely occur, while the simulated MSE is itself an average dominated by rare draws. In these designs the finite-sample MSE gap is therefore difficult to estimate precisely, and the discrepancy between it and the full-moment prediction reflects both the approximation and tail undersampling; we do not separate the two. The sign of the comparison agrees in every design.

\begin{table}[htbp]
\centering
\caption{Second-order prediction $1+\Delta/n$ at $n=500$ for the full lognormal law and for its truncation at the $q$ quantile, against the simulated MSE ratio.}
\label{tab:tail}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lccccc}
...
The simulations of Section \ref{sec:feasible} are implemented in R (\texttt{r/gamma0\_sim.R}). For each replication, the design matrices are built exactly as \texttt{lm\_robust(y \textasciitilde{} w + x)} and \texttt{lm\_lin(y \textasciitilde{} w, covariates = \textasciitilde{} x)} build them, and the fits are computed by \texttt{estimatr}'s own fitting routine, the function those two interfaces call; intervals use the degrees of freedom \texttt{estimatr} reports. The main runs use \texttt{estimatr} \gz{estimatr:v1}; the rare-value design is repeated with \texttt{estimatr} \gz{estimatr:v2}. The validation script \texttt{r/validate\_estimatr.R} and the Python cross-check \texttt{simulations/crosscheck\_r.py} establish the following, under both versions.
\begin{itemize}
\item On \gz{val:datasets} simulated datasets from four designs, the formula interfaces \texttt{lm\_robust} and \texttt{lm\_lin} and the simulation engine give identical estimates, HC1--HC3 standard errors and degrees of freedom (largest relative difference \gz{val:formulagap}), and a base-R matrix implementation of OLS, leverage and HC1--HC3 agrees to \gz{val:handgap}.
...
Section \ref{sec:estimand} also uses the Gaussian designs of the correction study: $X_i\sim N(0,I_p)$, $Y_i(0)=0.9\sum_jX_{ij}+\varepsilon_i(0)$ and $Y_i(1)=Y_i(0)-\varepsilon_i(0)+\varepsilon_i(1)+1+0.3X_{i1}$ with $N(0,1)$ residuals that are either shared or independent across arms, at $n=500$, $p=5$ and $n=1000$, $p\in\{2,5,10\}$. The calibration diagnostic uses three designs, all with $n=500$. \emph{Large $R^2_\tau$}: $p=5$, $X_i\sim N(0,\Sigma)$ with $\Sigma_{jk}=0.5^{|j-k|}$, $Y_i(0)=0.9\sum_jX_{ij}+\varepsilon_i$ and $Y_i(1)=Y_i(0)+1+3X_{i1}$ with $\varepsilon_i\sim N(0,1)$, so that $G=9$ and $R^2_\tau=9/13$; \gz{corr:large_hte_n500_p5:reps} replications. \emph{Small $R^2_\tau$}: the heavy-tailed variant of the introductory example ($X\sim\mathrm{LogNormal}(0,1.8^2)$, $\sigma(X)=1+1.5X$, treatment-effect slope $0.3$); \gz{corr:heavy_tail_example:reps} replications. \emph{$\gamma=0$}: the design of Table \ref{tab:gamma0-skew} with $s=2$; \gz{corr:lognormal_s2.0_prop:reps} replications. The first design was simulated with the NumPy implementation of Appendix \ref{sec:r-validation}, the second with the implementation used for the introductory example, and the third with \texttt{estimatr} \gz{estimatr:v1}. Monte Carlo standard errors are at most \gz{corr:mcse:max} percentage points.
