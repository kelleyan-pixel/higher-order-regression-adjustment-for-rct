...
Table \ref{tab:battery} compares all four procedures across \gz{hte:ba:ndesigns} designs in which the two mechanisms can coexist. IREG is paired with the plug-in correction, its appropriate implementation for the population ATE; REG needs no correction. The battery is weighted towards small heterogeneity, the case we expect in practice, and ranges from Gaussian designs to skewed, heavy-tailed and rare-value covariates, with MSE ratios from \gz{hte:ba:mse:min} to \gz{hte:ba:mse:max}. The scalar lognormal designs are those of Section \ref{sec:gamma0} with a treatment-effect slope added; the others reuse the designs of the correction study (Appendix \ref{sec:supp-inference}). All have $n=500$ and \gz{hte:reps} replications.

\begin{table}[htbp]
\centering
\caption{Coverage (\%) of nominal $95\%$ intervals for the population ATE, with median interval width in parentheses, for IREG with the plug-in correction and for REG, each with HC1 and HC3; the MSE ratio compares the point estimators ($n=500$; ``het.''\ denotes heteroskedastic noise). Designs are defined in Appendix \ref{sec:supp-inference}.}
\label{tab:battery}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{lrr|cc|cc}
\toprule
& & MSE ratio & \multicolumn{2}{c|}{\textbf{IREG} + plug-in} & \multicolumn{2}{c}{\textbf{REG}} \\
Design & $R^2_\tau$ & IREG/REG & HC1 & HC3 & HC1 & HC3 \\
\midrule
...
With little or no heterogeneity, every procedure covers at least $93\%$ at the $95\%$ level: IREG+HC1 with the plug-in covers between \gz{hte:ba:small:I1c:min}\% and \gz{hte:ba:small:I1c:max}\%, and REG+HC1 between \gz{hte:ba:small:R1:min}\% and \gz{hte:ba:small:R1:max}\%. The plug-in is not, however, estimating the population component. In the non-Gaussian designs its average is \gz{hte:ba:smallng:plug:min} to \gz{hte:ba:smallng:plug:max} times $G/n$, and adding the true $G/n$ instead leaves IREG+HC1 well below nominal (\gz{hte:ba:ln20_small:IREG_HC1o:cal:0.95}\% in the lognormal design with $s=2$; Table \ref{tab:plugin-cal}). The plug-in's upward bias happens to offset the leverage-driven understatement of HC1 documented in Section \ref{sec:gamma0}, and the resulting coverage is bought with width. The IREG+HC1 interval with the plug-in is up to \gz{hte:ba:small:wr1:max} times as wide as REG+HC1's at the median, and IREG+HC3 with the plug-in, which overcovers (up to \gz{hte:ba:small:I3c:max}\%), is up to \gz{hte:ba:small:wr3:max} times as wide as REG+HC3's. REG reaches the same coverage with shorter intervals, consistent with its smaller MSE in these designs. In the Gaussian design the plug-in's bias is small (\gz{hte:ba:gauss_low:plugratio:fmt} times $G/n$) and the two estimators' intervals have the same width.
...
Table \ref{tab:plugin-cal} also answers the natural question of why one should not simply always add the plug-in to IREG. With small $R^2_\tau$ the corrected interval is miscalibrated across nominal levels: in the skewed designs it undercovers at $80\%$ (\gz{hte:ba:ln20_small:IREG_HC1c:cal:0.80}\% with $s=2$) and reaches or exceeds nominal at $99\%$ (\gz{hte:ba:ln20_small:IREG_HC1c:cal:0.99}\%), a crossing pattern much like REG+HC1's and unlike the uniform undercoverage of the oracle interval. Its $95\%$ coverage therefore reflects positive noise rather than the population component, and it buys no better calibration than REG+HC1 at a larger width. In the lognormal designs, IREG+HC3 without the plug-in is closer to nominal at the low levels (Appendix \ref{sec:supp-inference}).

Once heterogeneity is non-negligible the plug-in tracks $G/n$ (\gz{hte:ba:ln15_mid:plugratio:fmt} and \gz{hte:ba:ln15_large:plugratio:fmt} times $G/n$ in the two lognormal designs below) and no longer masks the leverage problem. In the lognormal designs with $s=1.5$ and $R^2_\tau=\gz{hte:ba:ln15_mid:R2}$ and \gz{hte:ba:ln15_large:R2}, IREG+HC1 with the plug-in covers only \gz{hte:ba:ln15_mid:IREG_HC1c:cov}\% and \gz{hte:ba:ln15_large:IREG_HC1c:cov}\%; HC3 raises this to \gz{hte:ba:ln15_mid:IREG_HC3c:cov}\% and \gz{hte:ba:ln15_large:IREG_HC3c:cov}\%, near nominal at moderate heterogeneity but not in the more extreme design. REG+HC1 undercovers there too, and by more (\gz{hte:ba:ln15_mid:REG_HC1:cov}\% and \gz{hte:ba:ln15_large:REG_HC1:cov}\%), because REG's residual retains the heterogeneity term $(W_i-\frac12)\gamma^\top Z_i$, which is heavy-tailed when the covariate is; HC3 improves REG but recovers only part of the gap (\gz{hte:ba:ln15_mid:REG_HC3:cov}\% and \gz{hte:ba:ln15_large:REG_HC3:cov}\%). The heavy-tailed design with $p=5$ and large heterogeneity shows the same ordering (IREG+HC3 with the plug-in \gz{hte:ba:heavy_high:IREG_HC3c:cov}\%, REG+HC1 \gz{hte:ba:heavy_high:REG_HC1:cov}\%, REG+HC3 \gz{hte:ba:heavy_high:REG_HC3:cov}\%). In the Gaussian design with large heterogeneity all four procedures are within a point of nominal, and in the rare-value design IREG is the more precise estimator (MSE ratio \gz{hte:ba:rare:mse}) while both IREG procedures remain slightly below nominal (\gz{hte:ba:rare:IREG_HC1c:cov}\% and \gz{hte:ba:rare:IREG_HC3c:cov}\%).

\subsection{Practical implications}
\label{sec:practical}

The three experiments point to a few conclusions. When $G=0$, IREG's inference can be fragile because the arm-specific fit concentrates leverage on extreme covariate values and HC1 does not account for it; in the same designs IREG is also the less precise point estimator. HC3 substantially reduces the problem in most designs but not in the heaviest tails, and it does nothing about the superpopulation component. As heterogeneity grows, $G/n$ becomes a genuine part of the uncertainty about the population ATE for centered IREG, and the plug-in tracks it. With the small heterogeneity we expect in practice, however, the plug-in is dominated by its own upward bias: its coverage gains offset leverage effects rather than estimate the component, and they come with wider intervals and no better calibration than REG+HC1. When skewed or heavy-tailed covariates occur together with non-negligible heterogeneity, every HC1 interval undercovers, REG's included, and HC3 helps both estimators without fully repairing either in the most extreme designs. Estimator choice and variance estimation therefore cannot be treated as separate decisions, and neither the simulations nor the theorem support a universal ranking of the two estimators.
...
The six designs above have $p\le3$ and independent coordinates. To check the formula in higher dimensions and with dependent coordinates, we add four covariate families at $p\in\{2,5,8\}$ (Table \ref{tab:families}). \emph{Gaussian} covariates favor IREG, with $D_p=-(p+1)G$. \emph{Rademacher} covariates, whose kurtosis is one, favor IREG strongly: $K=pI_p$ and $D_p=-(p+3)G$. A symmetric \emph{scale mixture} of normals, in which $2\%$ of units have six times the standard deviation, favors REG through kurtosis alone: $K=(p+2)\tilde\kappa I_p$ with $\tilde\kappa=\EEE[s^4]/\EEE[s^2]^2$ the scale kurtosis, and $D_p=G\{(p+2)\tilde\kappa-(2p+3)\}$. Gaussian covariates with the \emph{quadratic misspecification} $c(X^\top X-\tr\Sigma)$, $c=0.3$ and $\Sigma_{jk}=0.3^{|j-k|}$, favor REG through the misspecification block: $\mathcal E=2c\Sigma$ and $D_p=-(p+1)G+32c^2\tr(\Sigma^2)$. Every family has homoskedastic $N(0,1)$ noise shared by the potential outcomes, and $G=0.49$ ($1.96$ for the Rademacher family). Because both estimators are invariant to invertible linear transformations of the covariates, we simulate whitened covariates; the correlation of $\Sigma$ enters only through $\gamma$ and, for the misspecified family, through $\tr(\Sigma^2)$. The targets are exact. Recomputing them from the matrix form of Theorem \ref{theo:deficiency} with $10^6$ Monte Carlo draws reproduces them to within \gz{fam:matrixcheck:maxrel}\%, the largest discrepancy occurring in the scale mixture, whose fourth moments are hard to estimate even from a million draws; exact targets avoid this source of error. With $n=\gz{fam:n}$ and $B=\gz{fam:B}$ replications per design, every estimate is within \gz{fam:maxabsz} standard errors of its target, and the control variate makes the standard errors \gz{fam:seratio:min} to \gz{fam:seratio:max} times smaller than those of the raw estimator on the same replications. Figure \ref{fig:validation-z} shows the $z$-statistics of all \gz{valall:n} checks (the designs of Table \ref{tab:validation} at $n=3{,}200$ and those of Table \ref{tab:families}), of which \gz{valall:within196} are within $\pm1.96$; the two exceptions are the deviations of Table \ref{tab:validation} discussed above. Figure \ref{fig:validation-rates} shows the estimates against $n$. For the four families at $p=5$ the standard errors stay constant as $n$ grows, and one of the twelve points below $n=\gz{fam:n}$ lies outside $\pm1.96$ standard errors (the scale mixture at $n=1{,}000$, $z=\gz{fam:rate:heavytail1000:z}$), while its neighbors at $n=500$ and $n=2{,}000$ are within $\gz{fam:rate:heavytail:nbr}$ standard errors; with twenty-four family checks, one such deviation is expected under Monte Carlo error.
...
The battery of Section \ref{sec:combined} combines two families. The scalar lognormal designs are those of Section \ref{sec:gamma0} with a treatment-effect slope: $p=1$, $X_i\sim\mathrm{LogNormal}(0,s^2)$, $Y_i(0)=0.9X_i+\sigma(X_i)u_i$ and $Y_i(1)=Y_i(0)+1+gX_i$, with $\sigma(X)=1+X$ ($1+1.5X$ in the design with $s=1.8$, the heavy-tailed variant of the introductory example) and $(s,g)=(1.2,0.3)$, $(1.5,0.1)$, $(1.5,0.5)$, $(1.8,0.3)$, $(2,0.1)$, $(1.5,1)$ and $(1.5,3)$. The other designs reuse the $p=5$ designs of the correction study. \emph{Gaussian}: $X_i\sim N(0,I_5)$ (small heterogeneity) or $N(0,\Sigma)$ with $\Sigma_{jk}=0.5^{|j-k|}$ (large), with homoskedastic $N(0,1)$ noise. \emph{Misspecified}: $X_i\sim N(0,\Sigma)$ as above and $Y_i(0)=0.9\sum_jX_{ij}+0.2(X_{i1}^2-1)+0.15X_{i1}X_{i2}+u_i$. \emph{Skewed} and \emph{heavy-tailed}: $X_{i1}$ is a centered $\mathrm{LogNormal}(0,s^2)$ variable with $s=1.1$ or $s=1.5$, mixed with four independent standard normal coordinates by an AR(0.35) Cholesky factor and standardized, with $\sigma(X)=1+c|X_{i1}|+0.25\|X_i\|$ and $c=0.5$ or $0.75$. \emph{Rare extreme values}: $X_{i1}$ takes the value $\sqrt{(1-\rho)/\rho}$ with probability $\rho=0.02$ and $-\sqrt{\rho/(1-\rho)}$ otherwise, the other coordinates are standard normal, and the noise is homoskedastic. In these designs $Y_i(1)=Y_i(0)+1+\gamma^*X_{i1}$ with $\gamma^*=0.3$ and $3$ (Gaussian), $0$ (misspecified and skewed), $0.5$ and $3$ (heavy-tailed) and $1$ (rare values). The potential outcomes share their noise, and $R^2_\tau$ is computed from the design ($\EEE[\varepsilon^2]$ by $10^6$ Monte Carlo draws where it has no closed form). These simulations use the NumPy implementations of the introductory example and of the correction study, which apply the same OLS and HC formulas as \texttt{estimatr}; in the rare-value design the latter drops the HC3 contribution of a leverage-one observation, like the development version of \texttt{estimatr}. In the lognormal designs with small heterogeneity, IREG+HC3 without the plug-in covers between \gz{hte:ba:small:I3cal80:min}\% and \gz{hte:ba:small:I3cal80:max}\% at nominal $80\%$.
