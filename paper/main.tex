...
\date{September 2026}
\title{Higher-Order Asymptotics of \\ Regression-Adjusted Estimators in Randomized Trials\thanks{Code, simulation scripts and the computational certificates used in the proofs: \url{https://github.com/kelleyan-pixel/higher-order-regression-adjustment-for-rct/tree/v1.0.4} (release v1.0.4).}}
...
Assumption \ref{assu:moments} is a convenient sufficient condition rather than a sharp one: the coefficients of Theorem \ref{theo:deficiency} involve moments of $Z$ of order at most four and mixed moments such as $\EEE[Z\varepsilon^2]$, while $\MSEexp$ additionally requires the sixth moments of the moment vector $\mathcal V_i$, whose entries are products of at most two coordinates of $(X_i, Y_i)$. Assumption \ref{assu:moments} implies $\EEE\|\mathcal V_i\|^6<\infty$ by the Cauchy--Schwarz inequality.
...
\]
Here $W$ is an auxiliary Bernoulli$(1/2)$ variable independent of $(Z,\varepsilon(0),\varepsilon(1))$, and $\varepsilon=\varepsilon(W)$. Then, with $\MSEexp$ as in Definition \ref{def:msexp} and under the balanced CRT,
\[
...
\]
While $\tr(\mathcal E^2) = \|E\|_F^2 \geq 0$, the term $S^\top \EEE[Z\varepsilon^2]$ can have either sign, so $\Delta$ can be positive or negative in this case.

\begin{rema}[Bias and variance components]
\label{rema:bias}
Two of the terms in $D_p$ are a squared-bias difference; that regression adjustment has a bias of order $1/n$ is classical \citep{Freedman2008,Lin2013}. Let $\tilde\tau_n$ be the Taylor polynomial of Definition \ref{def:msexp}, expanded under complete randomization in the per-arm sample means of Lemma \ref{lemm:two_sample}. Its linear term has mean zero; its quadratic term has mean $\frac12\tr(H\Sigma_n)$, where $H$ is the Hessian of the estimator's coefficient map and $\Sigma_n$ the covariance of the per-arm means, which is exactly proportional to $1/n$; and its cubic term has mean $O(n^{-2})$, because third central moments of sample means are third cumulants divided by squared sample sizes. The expansion bias is therefore $\EEE[\tilde\tau_n-\tau]=\frac12\tr(H\Sigma_n)+O(n^{-2})$, and evaluating this leading term for the two estimators (Lemma \ref{lemm:bias} in Appendix \ref{sec:proofs}) gives
\[
...
\begin{coro}
\label{coro:homosk}
In addition to Assumption \ref{assu:moments}, suppose the potential outcomes satisfy a well-specified linear model with homoskedastic errors, $\EE{\varepsilon_i(w)\cond X_i}=0$ and $\EE{\varepsilon_i^2(w)\cond X_i}=\sigma_w^2$ for $w\in\{0,1\}$. Then $E = \mathcal E_W = 0$ and $\EEE[Z\varepsilon^2]=0$, so
\begin{equation}
...
The remaining term is nonnegative, since $8\nu^2\|S\|^2/(\|\gamma\|^2+4\nu^2p)\geq0$, and for $\nu>0$ it vanishes if and only if $S=\EEE[Z\|Z\|^2]=0$. Right-skewed covariates with a long upper tail, such as measures of user activity, engagement or spending, typically have large $\|S\|$, and the term can then dominate the covariate-shape block. The scalar case makes this transparent: with $p=1$, Corollary \ref{coro:mult_normproportional} gives
\[
\Delta
=
...
\paragraph{Estimating $D_p$.} Theorem \ref{theo:deficiency} suggests estimating $D_p$ and choosing between REG and IREG accordingly. We do not propose such a rule: $D_p$ depends on third and fourth moments and cross-moments, which a plug-in estimate recovers noisily, and choosing the estimator from the same sample creates a pretest problem whose effect on the final estimator is not described by the theorem.
...
\toprule
& & & \multicolumn{3}{c|}{\textbf{IREG} (\texttt{lm\char`\_lin})} & \multicolumn{2}{c}{\textbf{REG} (\texttt{lm\char`\_robust})} \\
...
\begin{table}[htbp]
\centering
\caption{Scale versus shape of the studentized statistic for IREG and REG in the $\gamma=0$, $s=2$ design of Table \ref{tab:gamma0-skew}. The implied scale $c$ solves $2F_{t}(q_{0.975}/c)-1=$ observed $95\%$ coverage; ``predicted'' is $2F_t(q/c)-1$ at each level; the last row of each block divides the empirical quantile of $|T|$ by the corresponding $t$ quantile.}
\label{tab:scale}
\small
...
\begin{table}[htbp]
\centering
\caption{Monte Carlo standard deviation and MSE of $\htau_\IREG$ and $\htau_\REG$ against their median HC1 and HC3 standard errors, in the designs of Table \ref{tab:gamma0-skew}; $\sqrt{V^*/n}$ is the common first-order standard deviation of the two estimators and $1+\Delta/n$ the second-order prediction of Theorem \ref{theo:deficiency}; Monte Carlo standard errors of the simulated MSE ratio are in parentheses, in units of the last digit.}
\label{tab:sampling}
\small
\setlength{\tabcolsep}{4pt}
...
In our designs the undercoverage of IREG+HC1 grows steadily with the covariate tail, well before any observation reaches leverage one, together with the largest within-arm leverage and the ratio of the HC3 to the HC1 standard error (Tables \ref{tab:gamma0-skew} and \ref{tab:gamma0-width}). Comparing the two standard errors of $\htau_\IREG$ is therefore a simple descriptive check of how sensitive the reported uncertainty is to leverage; we do not propose a threshold. Comparisons between the two estimators use the same variance estimator (Tables \ref{tab:gamma0-skew}, \ref{tab:sampling} and \ref{tab:gamma0-width}); pairings such as IREG+HC3 against REG+HC1 mix the choice of adjustment with the choice of variance estimator and are not head-to-head comparisons. None of this is a general ranking of the two estimators.
...
\[
\Sigma^{\mathcal V}_{ij}=\Cov(\mathcal V_i,V_j),
\qquad
\mu^{\mathcal V}_{3,ijk}=\EEE[(\mathcal V_i-\mu_i)(V_j-\mu_j)(V_k-\mu_k)]
\]
...
\label{lemm:matrix}
With $K$, $S$, $S_\gamma$, $E$ and $\mathcal E_W$ as in Theorem \ref{theo:deficiency},
\[
G=\|\gamma\|^2,
\quad
F=\gamma^\top K\gamma,
...
Section \ref{sec:estimand} also uses the Gaussian designs of the correction study: $X_i\sim N(0,I_p)$, $Y_i(0)=0.9\sum_jX_{ij}+u_i(0)$ and $Y_i(1)=Y_i(0)-u_i(0)+u_i(1)+1+0.3X_{i1}$ with $N(0,1)$ residuals, in five designs: residuals shared across arms at $(n,p)=(500,5)$, $(1000,2)$, $(1000,5)$ and $(1000,10)$, and independent across arms at $(n,p)=(500,5)$. The calibration diagnostic uses three designs, all with $n=500$. \emph{Large $R^2_\tau$}: $p=5$, $X_i\sim N(0,\Sigma)$ with $\Sigma_{jk}=0.5^{|j-k|}$, $Y_i(0)=0.9\sum_jX_{ij}+u_i$ and $Y_i(1)=Y_i(0)+1+3X_{i1}$ with $u_i\sim N(0,1)$, so that $G=9$ and $R^2_\tau=9/13$; \gz{corr:large_hte_n500_p5:reps} replications. \emph{Small $R^2_\tau$}: the heavy-tailed variant of the introductory example ($X\sim\mathrm{LogNormal}(0,1.8^2)$, $\sigma(X)=1+1.5X$, treatment-effect slope $0.3$); \gz{corr:heavy_tail_example:reps} replications. \emph{$\gamma=0$}: the design of Table \ref{tab:gamma0-skew} with $s=2$; \gz{corr:lognormal_s2.0_prop:reps} replications. The first design was simulated with the NumPy implementation of Appendix \ref{sec:r-validation}, the second with the implementation used for the introductory example, and the third with \texttt{estimatr} \gz{estimatr:v1}. Monte Carlo standard errors are at most \gz{corr:mcse:max} percentage points.
