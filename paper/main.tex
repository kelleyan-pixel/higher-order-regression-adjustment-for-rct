...
\date{September 2026}
\title{Higher-Order Asymptotics of \\ Regression-Adjusted Estimators in Randomized Trials\thanks{Code, simulation scripts and the computational certificates used in the proofs: \url{https://github.com/kelleyan-pixel/higher-order-regression-adjustment-for-rct/tree/v1.0.6} (release v1.0.6).}}
...
Y_i(1)=Y_i(0)+1,
\]
with $u_i\overset{\mathrm{iid}}{\sim}N(0,1)$. There is no treatment-effect heterogeneity, so the heterogeneity share $R^2_\tau$ of Section \ref{sec:asymptotics} is zero, and the whitened covariate has skewness \gz{intro:skew}. Our second-order formula nevertheless gives an asymptotic deficiency of $\Delta=\gz{intro:Delta}$ for IREG relative to REG, driven entirely by the interaction of covariate skewness with heteroskedasticity: asymptotically, using IREG costs about \gz{intro:Deltaround} users' worth of precision. In simulation, IREG's mean-squared error exceeds REG's by \gz{intro:sim:500}\% at $n=500$ (Monte Carlo standard error \gz{intro:se:500}\%), \gz{intro:sim:2000}\% at $n=2{,}000$ and \gz{intro:sim:8000}\% at $n=8{,}000$. The second-order prediction, \gz{intro:pred:500}\%, \gz{intro:pred:2000}\% and \gz{intro:pred:8000}\% respectively, overstates this gap, but the two converge as $n$ grows. Inference is affected too. At $n=500$, REG with HC1 standard errors covers the ATE in \gz{intro:cov:REG_HC1}\% of replications, but IREG with HC1 covers it in only \gz{intro:cov:IREG_HC1}\%; with HC3, IREG covers it in \gz{intro:cov:IREG_HC3}\% (Monte Carlo standard errors at most \gz{intro:cov:mcse} points). The design is the $s=1.1$ member of the family studied in Section \ref{sec:gamma0}.

Two features of this example recur throughout the paper. First, the effect is practically meaningful but bounded: a deficiency of about \gz{intro:Deltaround} observations is large relative to $n=500$ and negligible at $n=50{,}000$. Second, the size of the deficiency is governed by high-order moments of the covariate distribution, and those same moments govern how quickly the second-order approximation becomes accurate. If we instead take $X_i \sim \mathrm{LogNormal}(0,1.8^2)$ with $\sigma(X_i)=1+1.5X_i$ and a treatment-effect slope of $0.3$---a tail heavy enough that the whitened covariate has skewness $136$---then the formula returns $\Delta \approx 3.9\times10^4$ and predicts an MSE ratio of $78.5$ at $n=500$, whereas the simulated ratio is $1.68$. In that regime the expansion gets the ordering right and substantially overstates the numerical gap; Appendix \ref{sec:validity} quantifies where the approximation stops predicting the magnitude of the gap in a closely related scalar family, which adds a treatment-effect slope of $0.4$ to the design of the first example.
