...
- **Theorem 2** (second-order MSE difference) holds under Assumption 1 alone. Its proof (Appendix A) uses the
...
- The index algebra of Theorem 12 is certified symbolically by `verification/general_p_symbolic_certificate.py`
  (zero symbolic remainder for general `p`). The CRT shift `D_CRT = D_Bern + 4G` is checked numerically by
  `verification/crt_correction_certificate.py`. These are computer-assisted checks, not independent proofs.
- Remark 3 states the leading term of the expansion bias with an `O(n^-2)` remainder; its values come from evaluating
  the estimators' Hessians and are compared with simulated biases in Appendix B. The expansion MSE is not the exact
