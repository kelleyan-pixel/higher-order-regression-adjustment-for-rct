...
def gamma0_tables():
    ...
    for s_ in [0.6, 0.8, 1.0, 1.2, 1.5, 1.8, 2.0]:
        v = r[f"lognormal_s{s_:.1f}_prop"]
        skew = (exp(s_ ** 2) + 2) * sqrt(exp(s_ ** 2) - 1)
        cells = [cov(v, e) for e in ["IREG_HC1", "IREG_HC2", "IREG_HC3", "REG_HC1", "REG_HC3"]]
        rows.append(f"{s_:.1f} & {skew:.1f} & {v['median_max_leverage_IREG']:.2f} & "
                    + " & ".join(pct(c) for c in cells) + " \\\\")
    write("gamma0_skew_body.tex", rows)
    # --- other designs
    ...
    for j, (title, keys) in enumerate(blocks):
        ...
        rows.append(f"\\multicolumn{{8}}{{l}}{{\\emph{{{title}}}}} \\\\")
        for k in keys:
            v = r[k]
            n = v["design"]["n"]
            cells = [cov(v, e) for e in ["IREG_HC1", "IREG_HC2", "IREG_HC3", "REG_HC1", "REG_HC3"]]
    ...
    vals["val:formulagap"] = f"{max(V['formula_vs_engine_max_rel_gap'], V2['formula_vs_engine_max_rel_gap']):.0e}" \
        if max(V['formula_vs_engine_max_rel_gap'], V2['formula_vs_engine_max_rel_gap']) > 0 else "0"
    vals["val:handgap"] = f"{max(V['hand_vs_estimatr_max_rel_gap'], V2['hand_vs_estimatr_max_rel_gap']):.0e}"
    vals["val:crossgap"] = f"{max(C['max_rel_gap'].values()):.0e}"
    vals["val:crossn"] = str(C["datasets"])
    L1, L2 = V["leverage_one"], V2["leverage_one"]
    vals["val:lev:onem"] = f"{L1['one_minus_leverage']:.0e}"
    vals["val:lev:resid"] = f"{abs(L1['residual']):.0e}"
    ...
    vals["val:p1gap"] = f"{max(V['p1_closed_form_max_rel_gap'], V2['p1_closed_form_max_rel_gap']):.0e}"
...
CAL_METHODS = [("IREG_HC1", "HC1"), ("IREG_HC1c", "HC1 + plug-in"), ("IREG_HC1o", "HC1 + $G/n$ (oracle)"),
               ("IREG_HC3", "HC3"), ("IREG_HC3c", "HC3 + plug-in")]
...
def diagnostic_tables():
    ...
    for s_ in [0.6, 0.8, 1.0, 1.2, 1.5, 1.8, 2.0]:
        ...
        rows.append(f"{s_:.1f} & {I['sd']:.3f} & {Rg['sd']:.3f} & {I['mse'] / Rg['mse']:.2f} & {predtxt} & {I['median_se_HC1']:.3f} & "
                    f"{Rg['median_se_HC1']:.3f} & {I['median_se_HC3']:.3f} \\\\")
    ...
    tails = [("Heavy-tailed example ($s=1.8$)", 1.8, 1.5, 0.3, exj["heavy_tail_case"]["500"]["sim"], "heavy"),
             ("Table \\ref{tab:validity}, $s=1.5$", 1.5, 1.0, 0.4, exj["sweep"]["1.5"]["500"]["sim"], "t9"),
             ("Section \\ref{sec:gamma0}, $\\gamma=0$, $s=2$", 2.0, 1.0, 0.0,
              S["lognormal_s2.0_prop"]["sampling"]["IREG"]["mse"] / S["lognormal_s2.0_prop"]["sampling"]["REG"]["mse"], "g0")]
    rows = []
    fmt = lambda x: f"{x:.2f}" if x < 10 else f"{x:.1f}"
    for lab, sx, c, g, sim, key in tails:
        cells = [1 + population(sx, c, g, q)["Delta"] / 500 for q in (None, 1 - 1e-4, 1 - 1e-6, 1 - 1e-8)]
        rows.append(f"{lab} & {sim:.2f} & " + " & ".join(fmt(x) for x in cells) + " \\\\")
    ...
    for lab, name in (("IREG_HC1", "IREG, HC1"), ("IREG_HC3", "IREG, HC3")):
